...
class SpaceState:
    ...
    Public interface:
        ...
        trail_multiplier → float        — trail visibility multiplier
    ...
    def __init__(
        ...
        self._prev_state: str = self.IDLE
        ...
        # State-changed flag for one-shot effects
        self.just_entered: str | None = None
    ...
    def trail_multiplier(self) -> float:
        """Trail visibility multiplier for current state."""
        return Config.STATE_TRAIL_MULT.get(self._state, 1.0)

    @property
    def trail_alpha(self) -> int:
        """Background fade alpha for current state."""
        mapping = {
            self.IDLE: Config.TRAIL_ALPHA_IDLE,
            self.ACTIVE: Config.TRAIL_ALPHA_ACTIVE,
            self.EXCITED: Config.TRAIL_ALPHA_EXCITED,
            self.CALM: Config.TRAIL_ALPHA_ACTIVE,  # CALM uses ACTIVE alpha
        }
        return mapping.get(self._state, Config.TRAIL_ALPHA_IDLE)

    @property
    def is_active(self) -> bool:
        """Whether any hand interaction is happening."""
        return self._state in (self.ACTIVE, self.EXCITED, self.CALM)

    @property
    def is_idle(self) -> bool:
        """Whether the space is completely idle."""
